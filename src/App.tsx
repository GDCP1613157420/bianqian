...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setSyncMsg("同步中…");
      try {
        const { notes: merged } = await fullSync(notesRef.current, pat, getGistId(), (msg) => setSyncMsg(msg));
        setNotes(merged);
  ...
  const runSync = async (pat: string): Promise<boolean> => {
    ...
    setSyncMsg("同步中…");
    try {
      const { notes: merged } = await fullSync(notesRef.current, pat, getGistId(), (msg) => {
        setSyncMsg(msg);
  ...
  const handleTestPat = async () => {
    ...
    if (!pat) { setSyncError("请先粘贴 Token"); return; }
    setSyncError("");
    setSyncStatus("syncing");
    setSyncLog(["正在验证 Token…"]);
    try {
      const r = await testPat(pat);
      setSyncLog([`✅ 验证成功！账号: ${r.login}`, r.scopes ? `权限范围: ${r.scopes}` : "Fine-grained token，权限见 GitHub 设置页"]);
      setSyncStatus("success");
    } catch (e: any) {
      const msg = e?.message || "Token 验证失败";
      setSyncError(msg);
      setSyncLog([`❌ ${msg}`]);
      setSyncStatus("error");
    }
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[100]">
          <div className="bg-white rounded-xl shadow-2xl w-[300px] max-h-[80vh] overflow-y-auto">
            ...
            <div className="p-4 space-y-4 text-sm">
              <div>
                <div className="font-bold text-gray-700 mb-1">GitHub Token <span className="text-[10px] text-gray-400 font-normal">(粘贴后点 👁 查看确认)</span></div>
                <div className="relative">
                  <input
                    ...
                    className={`w-full border rounded px-3 py-2 pr-10 text-sm focus:outline-none focus:ring-2 font-mono ${
                  ...
                  <button
                    ...
                    className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-700 text-base"
                    title={showPat ? "隐藏 Token" : "显示 Token"}
                  >
                    {showPat ? "🙈" : "👁"}
                  </button>
                </div>
                {syncError && (
                  <div className="text-[11px] text-red-600 mt-1 bg-red-50 rounded px-2 py-1">❌ {syncError}</div>
                )}
              </div>
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                <button
                  ...
                  disabled={syncStatus === "syncing"}
                  className="px-3 py-2 border border-gray-300 text-gray-700 rounded hover:bg-gray-100 text-xs disabled:bg-gray-100"
                >
                  仅测试 Token
                </button>
                {getStoredPat() && (
                  <button onClick={handleClearSync} className="px-3 py-2 border border-red-300 text-red-600 rounded hover:bg-red-50 text-xs">清除</button>
                )}
              </div>
              {getStoredPat() && (
                <div className="text-[11px] text-gray-500 text-center">已绑定 · 上次同步：<b>{formatLastSync(getLastSyncTime())}</b></div>
              )}
              {syncLog.length > 0 && (
